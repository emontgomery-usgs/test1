function theResult = guido(theStruct, theFigureName, isModal)
...
%   most recent changes.  Fields named "help" are shown
%   in a separate dialog.  If theFigureName is not given
%   or isempty, the external name of theStruct will be
%   used.
...
if nargin > 0 & ischar(theStruct)
	...
	case '_cancel_'
		...
		delete(gcbf)
	otherwise
		error([' ## Unknown command: ' theCommand])
	end
	return
end
	...
	for i = 1:length(theFields)
		...
		case 'char'
			if ~isequal(theField, 'help')
				...
				theHintName = ['help ' theFigureName];
				theCallback = ...
