% defines elements of the targets used in arrangement 2
...
% new 5 - 8 rotate longer wavelength panels in NW quadrant
im = 5;
[m(im).xm, m(im).ym]= range_range(XA,YA,XB,YB,4.35,5.16,'w');
im = 6;
[m(im).xm, m(im).ym]= range_range(XA,YA,XB,YB,3.10,3.37,'w');
im = 7;
[m(im).xm, m(im).ym]= range_range(XB,YB,XC,YC,5.94,6.0,'w');
im = 8;
[m(im).xm, m(im).ym]= range_range(XB,YB,XC,YC,7.09,7.54,'w');
