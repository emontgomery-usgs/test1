...
%		to run SURFACE.exe (ALR)
% update 03-Jan-2001 14:18:52 - need to account for place holders for
% missing ens numbers
%     created by fixEns.m
...
switch numRawFile
    case 1
        ...
        if any(rawdata == '*')
            ...
            [dataPath, dataname, ext]=fileparts(rawdata);
            % mlint wants to fix the next line, it is OK
...
if isnan(ADCP_offset)
    ...
    f=netcdf(cdfFile);
    ADCP_offset=f{'D'}.transducer_offset_from_bottom(:);
    close(f)
end
...
if ~isequal(Dmin,str2double(dlgresult{1})) || ~isequal(Dmax,str2double(dlgresult{2}))
    Dmin=str2double(dlgresult{1});
    Dmax=str2double(dlgresult{2});
