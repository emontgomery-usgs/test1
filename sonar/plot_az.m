...
if ~exist('fname','var')
    ...
    if exist(fname, 'file') < 2 || isempty(fname)
      ...
      if(do_raw)
         ...
         imin_range = 100; % ignore data at short range
         ...
         i=1;
            for irot=1:length(rots)
               ...
               [bst,prt]=max(rawf,[],2);
               % remove values with low backscatter
               prt(find(bst<20))=NaN;
               bs(i,irot,:)=bst;
      ...
      % as read, all these matrices are 8x60x440- too big!  have to just 
      % use one image for debugging
      x1=x(1,:,:); y1=y(1,:,:); za1=za(1,:,:); ya1=ya(1,:,:);
      pr1=pr; profile_range1=profile_range; headangle1=headangle;
      clear x y za ya pr profile_range headangle
      x=x1; y=y1; za=za1; ya=ya1;
      pr=pr1; profile_range=profile_range1; headangle=headangle1;
      clear x1 y1 za1 ya1 pr1 profile_range1 headangle1
