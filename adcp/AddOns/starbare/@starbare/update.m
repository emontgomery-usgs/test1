...
% starbare/update -- Update the "StarBare" window.
...
for k = 1:len
	theDataVariables{k} = theData{theVariables{k}};
...
for k = 1:len
	isVelocity(k) = any(findstr(name(theDataVariables{k}), 'vel'));
end
...
isAllSameKind = 1;
for k = 2:len
	a = name(theDataVariables{k-1});
	b = name(theDataVariables{k});
	a(length(a)) = '';
	b(length(b)) = '';
...
for k = 1:len
	v = theDataVariables{k};
	w = theMaskVariables{k};
	if ischar(v), v = theData{v}; end
	if ischar(w), w = theData{w}; end
	if ~isempty(v)
		x{k} = e;
		s = ncsize(v);
		if length(s) == 1
			z{k} = v(e); zz{k} = w(e);   % zz is the mask.
		...
		elseif ~isEnsembleAveraging
			...
			y{k} = b;
			indices = min(e):max(e);
			z{k} = v(indices, b); zz{k} = w(indices, b);
			if ~isempty(z{k})
				theFillValue = fillval(v);
				if ~isempty(theFillValue)
					z{k}(z{k} == theFillValue) = NaN;
		...
		if length(s) == 1
			...
			if ~isempty(z{k})
				...
				case logical(1)   % Images and contours.
					if length(b) > 1
						hasDepth = 1;
						p = pximage(x{k}, y{k}, z{k});   % PXImage object.
						p.s = zz{k};
						h = px(p);   % Image handle.
						theImages(k) = h;
						if isContour
...
pxset(self, 'itsImages', theImages);
