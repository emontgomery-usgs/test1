function theResult = doupdate(self)

% starbuck/doupdate -- Update the "StarBuck" window.
%  doupdate(self) updates the window associated with self,
%   a "starbuck" object.
...
theTimeSampling = psget(self, 'itsTimeSampling');
...
theVelocities = {'u_1205', 'v_1206', 'w_1204', 'Werr_1201'};

isVelocity = zeros(1, length(theVariables));
for k = 1:len
	for i = 1:length(theVelocities)
		if any(findstr(name(theVariables{k}), theVelocities{i}))
			isVelocity(k) = 1;
			break
		end
	end
...
isEnsembleAveraging = psget(self, 'itIsEnsembleAveraging');
theEnsembleStart = psget(self, 'itsEnsembleStart');
...
theRotationAngle = psget(self, 'itsRotationAngle');   % degrees.
theColorFactor = psget(self, 'itsColorFactor');

% Adjust scrollbars.

h = findobjs(gcf, 'Style', 'slider', 'Tag', 'bottom');
set(h, 'Value', theEnsembleStart)
theSliderStep = min(1, theEnsembleStep*theEnsembleCount/theEnsembleMax);
set(h, 'SliderStep', [0.25 1] * theSliderStep)

h = findobjs(gcf, 'Style', 'slider', 'Tag', 'right');
set(h, 'Value', theBinStart);
theSliderStep = min([0.25 1], [1 theBinCount]*theBinStep/theBinMax);
set(h, 'SliderStep', theSliderStep)
	...
	if ~isempty(v)
		...
		if length(s) == 1
			...
			if ~isempty(theFillValue)
				z{k}(z{k} == theFillValue) = NaN;   % Use "autonan" feature.
			end
			if isVelocity(k), z{k} = z{k}; end   % cm/s.
		elseif ~isEnsembleAveraging
			y{k} = b; z{k} = v(e, b).';   % <=== NEEDS FIXING.
			if ~isempty(z{k})
				theFillValue = fillval(v);
				if ~isempty(theFillValue)
...
if ~isScatter & ~isProgressive & ~isempty(theVariables)
...
if (isScatter | isProgressive) & isVelocities
	...
	theUnits = u.units(:);
	if isProgressive, theUnits = 'cm'; end
	s = name(u);
...
%  values.  (In StarBeam: yes; StarBuck, not yet.)
...
dotimeaxis(self)
if hasDepth, dodepthaxis(self), end
